...
def dashboard(request):
    ...
    # 2. APLICA LOS FILTROS DEL FORMULARIO AL QUERYSET BASE para los contadores
    queryset_para_contadores = base_queryset
    ...
    if filter_form.is_valid():
        ...
        # Aplicar filtros al queryset para contadores
        if q:
            queryset_para_contadores = queryset_para_contadores.filter(
                Q(radicado__icontains=q) | Q(asunto__icontains=q)
            )
        
        # FILTRO DE VIGENCIA PARA CONTADORES
        if vigencia and vigencia != 'todos':
            # Si se seleccionó un año específico, filtra por ese año
            queryset_para_contadores = queryset_para_contadores.filter(
                fecha_recepcion_inicial__year=vigencia
            )
        elif not vigencia and not estado:
            # Si no se seleccionó nada (valor por defecto), filtra por año actual
            current_year = date.today().year
            queryset_para_contadores = queryset_para_contadores.filter(
                fecha_recepcion_inicial__year=current_year
            )
        # Si vigencia es 'todos', no se aplica filtro de año
        ...
        if estado:
            queryset_para_contadores = queryset_para_contadores.filter(estado=estado)
    
    # 3. CALCULA LOS CONTEOS PARA LAS TARJETAS usando el queryset filtrado
    total_pqrs = queryset_para_contadores.count()
    conteo_recibido = queryset_para_contadores.filter(estado='Recibido').count()
    conteo_en_tramite = queryset_para_contadores.filter(estado='En Trámite').count()
    conteo_resuelto = queryset_para_contadores.filter(estado='Resuelto').count()
    
    # 4. CREA UNA COPIA PARA LA TABLA (mantén tu lógica actual de paginación)
    queryset_para_tabla = queryset_para_contadores
    
    # 5. Ordena el resultado final que se mostrará en la tabla.
    lista_ordenada = queryset_para_tabla.order_by('-fecha_recepcion_inicial')
    ...
    # 6. Prepara el contexto final para la plantilla.
    contexto = {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Extracción de Radicado (se queda igual)
                ...
                # --- ¡NUEVA LÓGICA MEJORADA PARA ASUNTO Y PETICIONARIO! ---
                # Primero, encontramos el bloque de texto general del asunto
                ...
                if match_bloque_asunto:
                    ...
                    # Ahora, buscamos el nombre en mayúsculas DENTRO de ese bloque
                    match_peticionario = re.search(r"([A-ZÁÉÍÓÚÑ]{2,}\s[A-ZÁÉÍÓÚÑ\s,]+[A-ZÁÉÍÓÚÑ])", bloque_asunto_crudo)

                    if match_peticionario:
                        # Extraemos el nombre del peticionario
                        peticionario_extraido = match_peticionario.group(1).strip().rstrip(',')

                        # ¡AQUÍ ESTÁ LA MAGIA!
                        # El nuevo asunto es el texto del bloque A PARTIR de donde empieza el nombre
                        asunto_recortado = bloque_asunto_crudo[match_peticionario.start():]

                        # Limpiamos los espacios en blanco del resultado final
                        asunto_extraido = " ".join(asunto_recortado.split())
                    else:
                        # Si no encontramos un nombre en mayúsculas, usamos el bloque completo como antes
                        peticionario_extraido = "" # No se encontró un peticionario claro
                        asunto_extraido = " ".join(bloque_asunto_crudo.split())

                if not asunto_extraido:
                    # Si la primera regla falla, podemos tener un fallback si es necesario
                ...
                if asunto_extraido:
                    ...
                    # ... (tu código para extraer la fecha se queda igual)
...
def crear_pqrs(request):
    ...
    if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # "Adjuntamos" el usuario actual al objeto ANTES de guardarlo.
                form.instance._creador = request.user

                # Ahora sí guardamos, y la señal recibirá la información del creador.
                nueva_pqrs = form.save() 
                
                # --- El resto de tu bloque para adjuntar el PDF se queda igual ---
...
def editar_pqrs(request, pqrs_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.instance._actor = request.user
            form.save()
...
def detalle_pqrs(request, pqrs_id):
    ...
    fecha_vencimiento_iso = pqrs.fecha_vencimiento.isoformat() if pqrs.fecha_vencimiento else None
    ...
    contexto = {
        ...
        'fecha_vencimiento_iso': fecha_vencimiento_iso, # <-- ¡AÑADE ESTA VARIABLE AL CONTEXTO!
    }
...
@login_required # <-- Añade este decorador si no lo tienes
...
def confirmar_pqrs(request, pqrs_id):
    if request.method == 'POST':
        ...
        # --- ¡CAMBIO 1: Informamos QUIÉN está confirmando! ---
        # Le pasamos el usuario al modelo para que lo use el historial.
        pqrs._actor = request.user
        
        # --- ¡CAMBIO 2: Guardamos el objeto completo! ---
        # Quitamos update_fields para que se ejecute toda la lógica del método save().
        pqrs.save()
        
        messages.success(request, f'PQRS con Radicado {pqrs.radicado} ha sido confirmada.')
        
        return redirect('detalle_pqrs', pqrs_id=pqrs.id)
        
    return redirect('detalle_pqrs', pqrs_id=pqrs_id)

def calcular_vencimiento_api(request):
    tipo_tramite_id = request.GET.get('tipo_tramite_id')
    fecha_inicial_str = request.GET.get('fecha_inicial')
    
    try:
        tipo_tramite = TipoTramite.objects.get(pk=tipo_tramite_id)
        fecha_inicial = date.fromisoformat(fecha_inicial_str)
        
        # Reutilizamos la misma lógica de cálculo de días hábiles
        dias_a_sumar = tipo_tramite.dias_plazo
        ...
        # Devolvemos la nueva fecha en formato ISO (YYYY-MM-DD)
        return JsonResponse({'nueva_fecha_vencimiento': fecha_actual.isoformat()})
